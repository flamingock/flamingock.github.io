...
### 📝 Auditable execution
Flamingock retains the latest state it recorded for each Change and provides issue handling for failures or uncertainty. This supports auditable evolution.
...
- **Audit Store** → where Flamingock retains its latest recorded state per Change (managed by Flamingock).
...
6. Updates Flamingock’s latest recorded state for each Change
