...
- the **Audit Store** retains Flamingock's latest recorded state per Change.
...
Before the transition, inspect the configured Mongock repository, table, or collection and record the source baseline: the relevant Change IDs and their final recorded states.

Run Flamingock once against a realistic pre-production copy, using either:

- normal application startup with Flamingock configured; or
- `flamingock execute apply --jar ./my-app.jar`.

Validate the run against the baseline and expected Target System effects:

| Log pattern                                                                     | Meaning                                          |
|---------------------------------------------------------------------------------|--------------------------------------------------|
| `Starting audit log migration from Mongock to Flamingock community audit store` | Import began only.                               |
| `Change applied [change=migration-mongock-to-flamingock-community ...]`         | The importer completed successfully.             |
| `Change skipped [change=<id> reason=already_applied]`                           | That Change was not re-executed.                 |
| `Change applied [change=<id> duration=...]`                                     | A Change completed successfully.                 |
| `Stage completed [stage=... duration=... applied=... alreadyApplied=...]`       | The stage completed, including aggregate counts. |
| `Flamingock execution report — SUCCESS`                                         | The whole pipeline succeeded.                    |

A fully up-to-date stage might not log one individual skipped line per Change. In that case, use the stage aggregate counts and execution report to confirm the result.

Treat these patterns as failures or blockers that require investigation:

- `Ignored audit entry ... no matching change was found...`
- `Change failed [...]`
- `ABORTED STAGE ... Manual intervention required...`
- `Flamingock execution report — FAILED`

### Community baseline verification

This baseline is available to all Flamingock users. Compare the pre-transition Mongock source baseline with the execution logs and observed Target System effects, then check for inconsistent states:
...
A successful migration preserves the source-baseline Changes without re-executing them, applies only the previously pending Changes, and leaves `flamingock issue list` with no issues.

Practical verification flow:

1. Record the Mongock source baseline before the transition.
2. Run the application once with Flamingock enabled.
3. Compare execution logs with the baseline to confirm previously executed Changes were skipped and pending Changes were applied.
4. Confirm the Target System has no duplicate effects for executed Changes and has the expected effects for pending Changes.
5. Run `flamingock issue list --jar ./my-app.jar` and confirm no inconsistent audit states.

### Cloud and Self-Hosted historical verification (Enterprise)

Flamingock Cloud and Self-Hosted Edition provide `flamingock audit list` for more complete historical verification. Use it to inspect the current audit state or the full chronological history in addition to the Community baseline:
...
If your application needs profiles, datasource URLs, or other runtime arguments, pass them through the CLI as well. For an issue, request guidance with:

```bash
flamingock issue get --jar ./my-app.jar -c <change-id> --guidance
```

CLI execution must return code `0`. Normal application startup must complete successfully and show `SUCCESS`.
